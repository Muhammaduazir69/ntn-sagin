 ...
 * (OpenSkyMobilityModel) receives REAL UDP downlink traffic from a passing
 * LEO satellite. The aircraft cruises (trace built inline in the OpenSky
 * schema) while the satellite crosses overhead; a per-second probe forms the
 * satellite→aircraft SNR from FSPL at the live geometry, maps it to a packet-
 * error rate, and updates the propagation delay. Delivered goodput tracks the
 * pass.
 *
 * Covers the OpenSkyMobilityModel / OpenSkyAdsbTrace classes with a real data
...
LinkProbe()
{
    ...
    g_lastRx = tot;
    std::printf("  %6.1f  (%9.0f,%6.0f)  %7.2f  %7.2f  %9.3f\n",
                Simulator::Now().GetSeconds(), u.x, u.z, elev, snr, mbps);
...
main(int argc, char* argv[])
{
    ...
    // Build an ADS-B trace inline: a great-circle cruise leg heading east at
    // cruiseSpeedMps and cruiseAltM, sampled every 10 s, from a reference.
    ...
    for (double t = 0.0; t <= simSeconds + 20.0; t += 10.0)
    {
        ...
        smp.lat_deg = refLat;
        smp.lon_deg =
            refLon + mEast / (111320.0 * std::cos(refLat * M_PI / 180.0));
    ...
    NodeContainer nodes;
    nodes.Create(2); // 0=aircraft terminal, 1=satellite
    Ptr<sagin::OpenSkyMobilityModel> ac =
        CreateObject<sagin::OpenSkyMobilityModel>();
    ac->SetReference(refLat, refLon, 0.0);
    ac->SetTrace(trace);
    nodes.Get(0)->AggregateObject(ac);
    g_ac = ac;
    ...
    ipv4.SetBase("10.33.1.0", "255.255.255.0");
    ...
    std::printf("# sagin-adsb-flight-leo-traffic (ADS-B aircraft terminal + LEO)\n");
    std::printf("#   sim=%.0fs leoAlt=%.0fkm freq=%.0fGHz load=%.1fMbps "
                "EIRP=%.1fdBm cruise=%.0fm/%.0fm/s\n",
                simSeconds, leoAltKm, freqGHz, dataRateMbps, g_eirpDbm,
                cruiseAltM, cruiseSpeedMps);
    std::printf("# %5s  %18s  %7s  %7s  %9s\n",
                "t_s", "ac_xz_m", "elev", "snr_dB", "goodput");
